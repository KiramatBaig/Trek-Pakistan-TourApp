...
class HotelDel extends StatefulWidget {
  final String name;

  HotelDel(this.name);
  @override
  _HotelDelState createState() => _HotelDelState();
}


class _HotelDelState extends State<HotelDel> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<QuerySnapshot>(
        future: FirebaseFirestore.instance.collection('hotels').where('title',isEqualTo: widget.name).get(),
                          ...
                          children: <Widget>[
                            SizedBox(
                              ...
                              child: RaisedButton(
                                ...
                                child: Text("Delete Hotel", style: TextStyle(
                                ...
                                onPressed: ()async {
                                  ...
                                  var querySnapshots = await collection.where('title',isEqualTo: widget.name).get();
                                  for (var snapshot in querySnapshots.docs) {
                                    print(snapshot.id);
                                    DocumentReference documentreference = FirebaseFirestore.instance.collection('hotels').doc(snapshot.id);
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: Padding(
                                            ...
                                            child: RectangluarButton(color: Colors.lightBlueAccent.withOpacity(0.7),icon: Icon(Icons.location_on),press: (){},text: "Location: ${list[0]['destinationName']}",),
                                          ),
                                        ),
                                      ],
                                    ),
